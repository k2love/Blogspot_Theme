...
(function() {
    ...
    function onPlayerStateChange(event) {
        ...
        if (event.data === YT.PlayerState.PAUSED) {
            // 시간 차이가 0.5초 미만일 때만 정지 버튼에 의한 일시정지로 간주
            if (timeDiff < 0.5) {
                isPlaying = false;
    ...
    function updateVideoPosition() {
        ...
        // wasPlayingBeforePause가 true이고 현재 정지 상태라면 Sticky 해제
        if (wasPlayingBeforePause && !isPlaying) {
        ...
        if (isPlaying) {
            ...
            // 시간 이동 등 다른 동작으로 인한 일시정지일 경우 Sticky 유지
            placeholder.style.height = `${videoHeight}px`;
            videoContainer.style.maxWidth = `${wrapper.offsetWidth}px`;
        }
    }
