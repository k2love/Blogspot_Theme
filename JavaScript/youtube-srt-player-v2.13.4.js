...
(function () {
    ...
    // DOMContentLoaded 이벤트 핸들러에 미니 플레이어 버튼 이벤트 리스너 추가
    ...
    function toggleVideoExpand() {
        ...
        // 미니 플레이어 모드일 경우 먼저 해제
        if (videoContainer.classList.contains('mini-player')) {
            toggleMiniPlayer(); // 미니 플레이어 모드 해제
            return; // 함수 종료 - 원래 모드로 돌아감
        }
    ...
    async function initializePlayer() {
        try {
            ...
            if (expandButton) {
                expandButton.addEventListener('click', toggleVideoExpand);
                console.log('확장 버튼 이벤트 리스너 설정됨');
            } else {
                console.warn('확장 버튼을 찾을 수 없음');
            }
            
            const miniPlayerBtn = document.querySelector('[data-target="mini-player"]');
            if (miniPlayerBtn) {
                miniPlayerBtn.addEventListener('click', toggleMiniPlayer);
                console.log('미니 플레이어 버튼 이벤트 리스너 설정됨');
            } else {
                console.warn('미니 플레이어 버튼을 찾을 수 없음');
            }

            // 스크롤 및 리사이즈 이벤트 리스너 설정
    ...
    function addStyles() {
        ...
        .video-container.mini-player .subtitle-overlay {
            transform: scale(0.5); /* 자막 크기 줄임 */
            transform-origin: center bottom; /* 변형 기준점 */
        }

        .video-container.mini-player #subtitle-text-ko span {
        font-size: 14px; /* 더 작은 폰트 크기 */
        max-width: 90%;
        }

        .video-container.mini-player #subtitle-text-en span,
        .video-container.mini-player #subtitle-text-extra span {
        ...
        .video-container.expanded {
            ...
            background-color: var(--widget-bg, #2d2d2d);
        }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        console.log('DOM 로드됨: YouTube 플레이어 초기화 시작');
        addStyles(); // 스타일 추가
        initializePlayer(); // 플레이어 초기화
    });

    // 디버깅 도우미
    window.debugYouTubePlayer = function() {
        console.log({
            player: player ? 'initialized' : 'not initialized',
            isPlaying,
            isMiniPlayer,
            expandButton: document.querySelector('[data-target="video-expand"]') ? 'found' : 'not found',
            miniPlayerButton: document.querySelector('[data-target="mini-player"]') ? 'found' : 'not found',
            videoContainer: document.querySelector('.video-container') ? 'found' : 'not found'
        });
    };
