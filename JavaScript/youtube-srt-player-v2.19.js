...
style.textContent = `
    ...
    /* 자막 기본 스타일 */
    .subtitle-toggle .subtitle-content {
        position: relative;
        font-size: 1em;
        width: 100%;
    }
    ...
    /* 확대된 자막 스타일 */
    .subtitle-toggle.zoomed .subtitle-content {
...
function toggleSubtitleZoom(container, button) {
    ...
    // 다른 모든 자막의 확대 상태 해제
    document.querySelectorAll('.subtitle-toggle').forEach(elem => {
        if (elem !== container) {
            elem.classList.remove('zoomed');
            const contentDiv = elem.querySelector('.subtitle-content');
            if (contentDiv) {
                resetSubtitleStyles(contentDiv);
            }
            const otherButton = document.querySelector(`[data-target="${elem.id}"].zoom-toggle-btn`);
            if (otherButton) {
...
// 자막 스타일 초기화 헬퍼 함수
function resetSubtitleStyles(contentDiv) {
...
function initializeSubtitleToggles() {
    ...
    toggleButtons.forEach((button) => {
        ...
        if (targetId === 'video-expand') return; // 확장 버튼 제외
        ...
        // 토글 버튼의 부모 요소에 줌 버튼을 먼저 추가
        button.parentNode.insertBefore(zoomButton, button);
