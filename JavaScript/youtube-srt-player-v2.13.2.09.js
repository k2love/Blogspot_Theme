...
(function () {
    ...
    function toggleVideoExpand() {
        console.log('toggleVideoExpand 함수 호출됨'); // 디버깅용
        const videoContainer = document.querySelector('.video-container');
        if (!videoContainer) {
            console.error('비디오 컨테이너를 찾을 수 없음');
            return;
    ...
    function initializeSubtitleToggles() {
     ...
     // 버튼 위치를 바꾸는 함수 수정 - 버튼 가시성 문제 해결
     function moveControlButtons() {
        console.log('moveControlButtons 함수 실행'); // 디버깅용
        ...
        // 컨트롤 가시성 클래스 추가
        leftButtonsContainer.classList.add('controls-visible');
        existingButtonsGroup.classList.add('controls-visible');

        // 미니 플레이어 버튼 생성 또는 업데이트
        ...
        if (!miniPlayerBtn) {
            ...
            miniPlayerBtn.innerHTML = '<i class="fas fa-compress"></i>';
            miniPlayerBtn.title = "미니 플레이어";
            miniPlayerBtn.style.display = 'flex'; // 강제 표시
            leftButtonsContainer.appendChild(miniPlayerBtn);
        }
        
        // 이벤트 리스너 제거 후 다시 추가 (중복 방지)
        const miniPlayerClickHandler = function(event) {
            event.preventDefault();
            console.log("미니 플레이어 버튼 클릭됨");
            toggleMiniPlayer();
            event.stopPropagation(); // 이벤트 버블링 방지
        };
        
        miniPlayerBtn.removeEventListener('click', miniPlayerClickHandler);
        miniPlayerBtn.addEventListener('click', miniPlayerClickHandler);        
        ...
        if (!expandBtn) {
            ...
            expandBtn.style.display = 'flex'; // 강제 표시
            leftButtonsContainer.appendChild(expandBtn);
        }
        
        // 이벤트 리스너 제거 후 다시 추가 (중복 방지)
        const expandClickHandler = function(event) {
            event.preventDefault();
        ...
        expandBtn.removeEventListener('click', expandClickHandler);
        expandBtn.addEventListener('click', expandClickHandler);
        
        // console로 확인
        console.log('미니 플레이어 버튼:', miniPlayerBtn);
        console.log('확장 버튼:', expandBtn);
        
        // 초기 가시성 설정 - 콜백으로 지연 실행하여 DOM에 완전히 추가된 후 실행
        setTimeout(() => {
            showControls(true); // 처음에는 영구적으로 표시
            console.log('컨트롤 가시성 설정 완료');
        }, 500);
    }
        ...
        try {
            ...
            if (videoContainer) {
                ...
                videoContainer.addEventListener('click', function(e) {
                    ...
                    if (target.dataset.target === 'mini-player') {
                        console.log('미니 플레이어 버튼 클릭 감지');
                        e.preventDefault();
                        toggleMiniPlayer();
                        e.stopPropagation();
                    } else if (target.dataset.target === 'video-expand') {
                        console.log('확장 버튼 클릭 감지');
                        e.preventDefault();
                        toggleVideoExpand();
                        e.stopPropagation();
                    }
            ...
            // 컨트롤 버튼 위치 변경 함수 즉시 실행하고 지연도 추가
            moveControlButtons();
            console.log('컨트롤 버튼 초기화 완료');
            
            // 버튼 이벤트가 확실히 적용되도록 추가 지연 및 재적용
            setTimeout(() => {
                moveControlButtons();
                showControls(true);
                ...
                if (miniPlayerBtn) {
                    console.log('미니 플레이어 버튼 이벤트 재설정');
                    miniPlayerBtn.onclick = function(e) {
                        e.preventDefault();
                        console.log('미니 플레이어 버튼 클릭 (인라인 핸들러)');
                        toggleMiniPlayer();
                        e.stopPropagation();
                        return false;
                    };
                ...
                if (expandBtn) {
                    console.log('확장 버튼 이벤트 재설정');
                    expandBtn.onclick = function(e) {
                        e.preventDefault();
                        console.log('확장 버튼 클릭 (인라인 핸들러)');
                        toggleVideoExpand();
                        e.stopPropagation();
                        return false;
                    };
                ...
                console.log('초기 컨트롤 가시성 설정 완료');
            }, 1000);
            
            // 5초마다 컨트롤 버튼 가시성 확인 및 재설정
            setInterval(() => {
                const playerControls = document.querySelector('.player-controls');
                const toggleButtons = document.querySelector('.toggle-buttons-group');
                
                if (playerControls && !playerControls.classList.contains('controls-visible')) {
                    playerControls.classList.add('controls-visible');
                ...
                if (toggleButtons && !toggleButtons.classList.contains('controls-visible')) {
                    toggleButtons.classList.add('controls-visible');
                ...
                if (miniPlayerBtn && !miniPlayerBtn.onclick) {
                    miniPlayerBtn.onclick = function(e) {
                        e.preventDefault();
                        toggleMiniPlayer();
                        e.stopPropagation();
    ...
    style.textContent = `
    /* CSS 스타일 - DOM 로드 시 추가되는 스타일에 아래 내용 추가 */
    ...
    .player-controls {
        ...
        pointer-events: none; /* 초기에는 클릭 이벤트 차단 */
    }
