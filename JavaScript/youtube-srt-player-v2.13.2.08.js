...
(function () {
    ...
    function initializeSubtitleToggles() {
     ...
     function moveControlButtons() {
        ...
        // 버튼 체크 - 이미 있는지 확인
        if (!document.querySelector('[data-target="mini-player"]')) {
        ...
        if (!document.querySelector('[data-target="video-expand"]')) {
    ...
    async function initializePlayer() {
        try {
            ...
            // 컨트롤 버튼 위치 변경 함수 지연 실행 (DOM이 완전히 로드된 후)
            setTimeout(() => {
                moveControlButtons();
