...
(function () {
    ...
    class YouTubePlayer {
        constructor(containerElement) {
            ...
            this.playerElement = this.container.querySelector(`#player${this.videoId.slice(-1)}`);
              if (!this.playerElement) {
                console.error(`플레이어 요소를 찾을 수 없음: player${this.videoId.slice(-1)}`);
                return;
            }
                ...
                return entries
                    .map((entry) => {
                        ...
                        const [startTime, endTime] = timeCode.split(' --> ').map((timeStr) => {
                            const [time, ms] = timeStr.trim().split(':').map(Number);
