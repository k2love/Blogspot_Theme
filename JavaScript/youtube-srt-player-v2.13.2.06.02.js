...
(function () {
    ...
    function toggleVideoExpand() {
        ...
        const wrapper = videoContainer.closest('.sticky-wrapper'); // 부모 wrapper 참조
        ...
        if (videoContainer.classList.contains('expanded')) {
            ...
            videoContainer.style.maxWidth = wrapper ? `${wrapper.offsetWidth}px` : ''; // wrapper 너비로 복원
            videoContainer.style.margin = '';
            videoContainer.style.left = ''; // 추가적인 위치 조정 초기화
            videoContainer.style.height = ''; // 세로 크기 초기화
    
            // Sticky 상태라면 wrapper 너비에 맞게 조정
            ...
            videoContainer.style.margin = '0 calc(-50vw + 50%)'; // 중앙 정렬
            videoContainer.style.left = '0'; // Sticky일 때 왼쪽 정렬 보정
            // 세로 크기는 비율에 맡김 (CSS에서 제어)
        }
    ...
    function updateVideoPosition() {
        ...
        if (rect.top < 0) {
            ...
            if (videoContainer.classList.contains('expanded')) {
                ...
                videoContainer.style.left = ''; // 기본 위치로
                videoContainer.style.margin = '';
            }
    ...
    /* 미니 플레이어에서 호버 시 컨트롤 버튼 표시 */
    .video-container.mini-player:hover .player-controls {
        opacity: 1; /* 호버 시 보이게 */
    ...
    .video-container.expanded .video-wrapper {
        position: relative;
        width: 100%;
        padding-bottom: 56.25%; /* 16:9 비율 강제 유지 */
        height: 0;
        overflow: hidden;
    }
